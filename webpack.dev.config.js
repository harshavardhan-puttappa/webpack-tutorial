...
module.exports = {
  ...
  output: {
    // filename: 'bundle.js',
    filename: '[name].js', // [name] is the entrypoints key
  ...
  // webpack-dev-server npm package configuration to see changes instantly
  devServer: {
    port: 9000,
    static: {
  ...
  module: {
    rules: [
      ...
      {
        test: /\.css$/,
        use: ['style-loader', 'css-loader'], // specifies to use two loaders for importing css,scss,less files
      },
  ...
  plugins: [
    // new TerserPlugin(), // not required in dev mode
    // new MiniCSSExtractPlugin({
    //   filename: 'styles.[contenthash].css', // not required in dev mode
    // }),
